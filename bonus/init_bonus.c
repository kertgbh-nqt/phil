...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/07 09:56:41 by mel-garr          #+#    #+#             */
/*   Updated: 2023/05/07 09:56:41 by mel-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_global_mutex(t_data *data)
{
	...
	if (pthread_mutex_init(&(data->food), NULL))
		return (-1);
	if (pthread_mutex_init(&(data->left_to_eat_m), NULL))
		return (-1);
	if (pthread_mutex_init(&(data->simu_status_m), NULL))
		return (-1);
	if (pthread_mutex_init(&(data->all_eat_m), NULL))
		return (-1);
	return (0);
}
