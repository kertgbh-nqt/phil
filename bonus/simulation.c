...
/*                                                +#+#+#+#+#+  
	+#+           */
/*   Created: 2023/05/07 08:18:08 by mel-garr          #+#    #+#             */
/*   Updated: 2023/05/07 08:18:08 by mel-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_eat(t_philo *philo)
{
	...
	new_usleep(philo->data->time_to_eat, philo->data);
	philo->left_to_eat++;
...
void	*simulation(void *arg)
{
	...
	if (philo->index % 2 && philo->data->philo_nbr > 1)
		usleep(10000);
	while (!philo->data->simu_status && philo->data->all_eat == 0)
	{
		philo_eat(philo);
		print_routine("is sleeping", philo, CAN_WRITE);
		new_usleep(philo->data->time_to_sleep, philo->data);
